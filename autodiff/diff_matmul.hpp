...
public:
    ...
    void pullGradient() override
    {
        ...
        // mLeft.mGradient += Matmul::matmul(grad, rightTranspose, mLeftProductAxis, mRightProductAxis).reduceSum(mReductionAxesLeft, true);

        Matmul::matmul<T>(grad, rightTranspose, &mLeft.mGradient, mLeftGradientSettings);

        // mRight.mGradient += Matmul::matmul(leftTranspose, grad, mLeftProductAxis, mRightProductAxis).reduceSum(mReductionAxesRight, true).reshape(mRight.refWildcardShape());
